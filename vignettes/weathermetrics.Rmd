...
As a second example, the `lyon` package is used. 

To convert to degress Celsius, use the `kelvin.to.celsius` function:

```{r}
data(lyon)
lyon$TemperatureK <- celsius.to.kelvin(lyon$TemperatureC) #create column with 
                                                          #temperature in K
lyon$DP_K <- celsius.to.kelvin(lyon$DewpointC)
lyon$T_KtoC <- kelvin.to.celsius(lyon$TemperatureK)
lyon$DP_KtoC <- kelvin.to.celsius(lyon$DP_K)
lyon
```
...
As a second example, the `norfolk` data set is used. 
